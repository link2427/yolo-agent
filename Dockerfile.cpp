...
# Symlinks are recreated, not copied, for the same reason as npm/npx/corepack
# above: COPY dereferences a symlink source. The installer links
# /usr/local/bin/pi -> /opt/pi/pi/pi, so copying the link would drop a lone
# payload into /usr/local/bin stripped from its siblings under /opt/pi/pi, and
# pi would fail before printing anything. (opencode is a single self-contained
# binary so it survives either way, but it is linked the same way for symmetry.)
RUN ln -sf /opt/opencode/opencode /usr/local/bin/opencode \
...
COPY --chmod=0755 docker/tests/check-seccomp.py /tmp/check-seccomp.py
