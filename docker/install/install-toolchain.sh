...
# clang can drive the same mingw sysroot through lld. Kept non-fatal: it is a
# convenience path, and the gcc cross frontend above is the supported one.
if clang++ --target=x86_64-w64-windows-gnu -O2 -std=c++17 "$tmp/probe.cpp" \
  ...
  echo ">> note: clang lld cross-link unavailable in this image; gcc cross frontend is used"
  sed -n '1,5p' "$tmp/clang.err" || true
fi
